...
namespace AtTheShop
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            latesttotal = helper.GetTotalInStockForShop(1);
        ...
        private void ProcessThisTag(object sender, TagEventArgs e)
        {
           ...
           {
               items = helper.GetAllProducts(items,1);
               this.RefreshListbox();
               SetPriceTags();
               timerCheck.Start();
                    ...
                    if (Convert.ToDecimal(persondata[3]) >= totalPrice)
                    {
                        ...
                        foreach (var item in items)
                        {
                            if (item.Quantity > 0)
                            {
                                ...
                                sqlstatements.Add(helper.GetItemToPurchase(item.ProductId, item.Quantity, 1));
                        ...
                        bool completed = helper.MakePurchase(Convert.ToInt32(persondata[0]), 1, totalPrice, sqlstatements);
                        if (completed == true)

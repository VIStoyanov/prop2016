...
namespace AtTheShop
{
    class DataHelper
    {
        ...
        {
            String sql = "Select products.Product_id, products.Product_desc, products.Price,IFNULL(shops_product.Quantity,0) AS INSTOCKFORSHOP,products.InStock,shops_product.Shop_Shop_id FROM products LEFT Join shops_product on (products.Product_id = shops_product.Products_Product_id AND shops_product.Shop_Shop_id = "+shopid+");"; //Change shop shop id
            MySqlCommand command = new MySqlCommand(sql, connection);
        ...
        {
            int latesttotal = this.GetTotalInStockForShop(2);
